...
def build_modify_profile(parent_window_name):
    name = parent_window_name.title()
    base_window = Toplevel()
    label_username = Label(text="Name of the user")
    username = Text(height=1,width=10)
    username.insert("1.0", name)
    pic_to_change = Button(text="Select picture to change", command=open)
...
def build_user_page(name):
    ...
    button_change_profile = Button(frame_buttons, text="change profile",width=15, height=2,bg="red", command="")
